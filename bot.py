...
def send_message(slack_id, slu_id):
    ...
    while n < MAX_ATTEMPTS:
        ...
        try:
            result = client.chat_postMessage(
                channel='C03BB9M2UNR', #slack_id
                text=reminder_message(slu_id)
            )
            if result['ok']:
                n_messages_sent += 1
    ...
    if n == MAX_ATTEMPTS:
        logging.warning("Number of max unsuccessful attempts reached. Message was not sent.")
...
if __name__ == '__main__':
    ...
    logging.info(f'{n_messages_sent} students have received a reminder message on SLU {SLU_ID}.')
