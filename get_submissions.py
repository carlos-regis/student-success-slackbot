    ...
    while page < MAX_ALLOWED_PAGES:
        ...
        while n < MAX_CONN_TRIES:
            try:
                ...
                # Portal is down
                logging.info(f"Connection to Submission Portal API failed on {n} try with error: {e}")
                # Waits 10 seconds until next try
                time.sleep(10)
                n += 1

        if n == MAX_CONN_TRIES:
            logging.warning("Number of max unsuccessful attempts reached. Connection not successful.")
            break
...
def get_submitted_slack_ids(slu_id: int, url: str) -> Set[str]:
    ...
    submitted_slack_ids = set(
        df.loc[
            (df.learning_unit == slu_id)
            & 
            (df.slackid.str.len() == 11), 
            'slackid'
        ]
    )
